...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Compute TopK_Targeted_Logit_Capture from compiled claim tokens and "
            "step0 logit delta evidence in steer_from_neuronpedia.json."
        )
    )
...
def main() -> None:
    ...
    metric_result = compute_topk_targeted_logit_capture(
        claims=claims,
        traces=traces,
        config=TopKTLCCalcConfig(
    ...
    output: Dict[str, Any] = {
        ...
        "TopK_Targeted_Logit_Capture": metric_result,
    }
...
def _resolve_output_path(
    *,
    ...
    return (
        ...
        / f"layer{layer_id}-feature{feature_id}-topk-targeted-logit-capture.json"
    )
