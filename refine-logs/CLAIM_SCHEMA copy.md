...
Claim = {
  ...
  context_condition: always | positive_only | conditional  # note: 对输出端假设，定义为: 在激活token上干预能看到因果、在无激活token上干预也能看到因果（这个地方需要再想一下）
...
`Comment`: 这个会不会覆盖不全？什么是task behavior
...
`Comment`: 这个倒是也还行吧，我最开始觉得这个好判，不过既然实验的时候要做增强和抑制，就放到这里了。不过这个东西感觉还是看Logits比较靠谱
...
`Comment`: 较窄会被llm理解为特定的条件、上下文吧，而且这个东西的定义就不是很明晰
`Insp`: 反过来说，一个假说，我可以进行拆分，这样针对上下文就可以检验了（输入端）
