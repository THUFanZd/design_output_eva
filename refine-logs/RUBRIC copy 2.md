...
- `CoreScore` 只放“支持该假说的正向证据”。
- `Control_Robustness` 更像 validity gate，不直接并入核心总分。原因是它的作用主要是检查这个 explanation-scoring pipeline 会不会奖励错配/宽泛假说，而不是提供正向支持证据。  # note: 倒也不一定，这一步相当于把错配当做baseline  # question: 这里宽泛假说我不懂你指什么
- 上面的权重不是“理论真值”，只是默认先验，反映设计原则：
  - `Targeted_Logit_Capture` 最重要，因为它最直接评估“假说预测的 target 是否真正吃到了主要 logits 变化”；
  - `Polarity_Accuracy` 第二，因为方向错了，假说就不成立；
  - `Context_Selectivity` 第三，因为它体现 specificity；  # note: 这个上下文的，可以先粗犷地：设计一个针对性的上下文，设计很多其他上下文，然后看看最终的指标是否有区别（这对输入输出端都有用）
  - `Generation_Agreement` 最后，因为 generation 证据是辅助而不是主证据。  # note: 这不就是放屁吗，这是个什么原因？要我说的话，一方面受采样影响，另一方面，原本存在很强的token，即便干预也不一定能战胜这样的token，所以在最终的输出中难以体现。另外，LLM judge 也会引入噪声。
- 最终权重应在小开发集上做一次性校准并冻结。校准目标不是单一指标，而是联合优化：
  1. hard-negative separation
  2. control false positive rate
  3. small-scale human pairwise agreement
- 如果没有开发集，先用上述默认值。
